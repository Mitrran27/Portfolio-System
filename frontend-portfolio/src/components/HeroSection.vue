<template>
  <section id="home" class="min-h-screen flex items-center px-6 pt-20 pb-12 relative overflow-hidden">
    <!-- Scan line effect -->
    <div class="scan-line" />
      ...
      <div class="grid lg:grid-cols-2 gap-12 items-center">
        ...
        <div class="space-y-6">
          ...
          <Transition name="fade-slide">
            <div v-if="info?.available_for_work" class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-green-500/40 bg-green-900/20 text-green-400 text-xs font-exo">
          ...
          <!-- Name -->
          <div>
            <p class="text-gray-500 font-exo text-sm mb-2 tracking-widest uppercase">Hello, I'm</p>
            <h1 class="font-orbitron font-black leading-tight" style="font-size: clamp(2.2rem, 5vw, 3.8rem)">
              <span class="text-white">{{ info?.name || 'Your Name' }}</span>
            </h1>
            <h2 class="font-orbitron font-bold mt-2" style="font-size: clamp(1rem, 2.5vw, 1.6rem)">
              <span class="text-cyan-400 neon-text">{{ displayedTitle }}</span>
              <span class="cursor-blink text-cyan-400">|</span>
            </h2>
          </div>

          <p class="text-gray-400 font-exo leading-relaxed max-w-lg" style="font-size:1rem">
            {{ info?.bio || 'Passionate developer crafting digital experiences.' }}
          </p>

          <!-- CTA Buttons -->
          <div class="flex flex-wrap gap-4 pt-2">
            <a href="#projects" class="btn-neon px-7 py-3 rounded-xl text-sm">
              View Projects
            </a>
            <a v-if="info?.resume_url" :href="info.resume_url" target="_blank"
              class="px-7 py-3 rounded-xl text-sm font-orbitron border border-fuchsia-400/50 text-fuchsia-400 hover:bg-fuchsia-400/10 transition-all">
              Resume ↗
            </a>
          </div>

          <!-- Location -->
          <p v-if="info?.location" class="text-gray-600 text-sm font-exo flex items-center gap-1.5">
        ...
        <!-- Right: Avatar + social icons -->
        <div class="flex flex-col items-center gap-8">
          <!-- Avatar ring -->
          <div class="avatar-ring">
            <div class="avatar-inner">
              <img v-if="info?.avatar_url" :src="info.avatar_url" alt="avatar" class="w-full h-full object-cover" />
              <div v-else class="w-full h-full flex items-center justify-center text-cyan-400 font-orbitron font-black" style="font-size:4rem">
                {{ info?.name?.[0] || '?' }}
              </div>
            </div>
          </div>
          ...
          <div class="flex justify-center gap-4">
            <template v-for="s in socials" :key="s.platform">
              <a :href="s.url" target="_blank" rel="noopener noreferrer"
                :title="s.platform"
                :class="['social-icon w-12 h-12 rounded-full card-glass flex items-center justify-center text-gray-400 transition-colors',
                  ['Twitter','X'].includes(s.platform) ? 'hover:text-fuchsia-400' : 'hover:text-cyan-400']">
                <SocialIcon :platform="s.platform" />
              </a>
            </template>
            <a v-if="info?.email" :href="`mailto:${info.email}`"
              class="social-icon w-12 h-12 rounded-full card-glass flex items-center justify-center text-gray-400 hover:text-fuchsia-400">
