    ...
    <div class="max-w-4xl mx-auto">
      <div class="text-center mb-12">
        <h2 class="font-orbitron font-bold text-white" style="font-size:clamp(1.6rem,4vw,2.8rem)">
          <span class="text-cyan-400 neon-text">Work</span>
      ...
      <div v-if="!experiences?.length" class="text-center text-gray-600 font-exo py-12">No experience entries yet.</div>

      <div v-else class="relative">
        <!-- Timeline line -->
        <div class="absolute left-6 top-0 bottom-0 w-px bg-gradient-to-b from-cyan-400/60 via-fuchsia-500/40 to-transparent hidden md:block" />
        ...
        <div class="space-y-6">
          <div v-for="(exp, i) in experiences" :key="exp.id"
            ...
            <div class="card-glass rounded-2xl p-5 hover:border-cyan-400/40 transition-all group">
              <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-2">
                <div>
                  <h3 class="text-white font-orbitron font-bold text-sm">{{ exp.role }}</h3>
                  <p class="text-fuchsia-400 text-sm font-exo mt-0.5">{{ exp.company }}</p>
                </div>
                <div class="flex items-center gap-2 flex-shrink-0">
                  <span class="text-gray-500 text-xs font-exo">{{ exp.period }}</span>
                  <span class="text-xs text-cyan-400 opacity-0 group-hover:opacity-100 transition-opacity font-exo">
                    View details →
                  </span>
                </div>
              </div>
              <ul v-if="exp.bullets?.length" class="space-y-1 mt-3">
                <li v-for="bullet in exp.bullets.slice(0,2)" :key="bullet"
                  ...
                  <span class="text-cyan-400 flex-shrink-0 mt-0.5">▹</span>{{ bullet }}
                </li>
                <li v-if="exp.bullets.length > 2" class="text-gray-600 text-xs font-exo pl-4">
    ...
    <Transition name="modal">
      <div v-if="selected" class="modal-backdrop" @click.self="selected = null">
        <div class="modal-box card-glass rounded-2xl overflow-hidden">
          <div class="flex items-start justify-between p-5 border-b border-cyan-400/20">
            <div>
              <h3 class="text-white font-orbitron font-bold">{{ selected.role }}</h3>
              <p class="text-fuchsia-400 text-sm font-exo mt-0.5">{{ selected.company }}</p>
            </div>
            ...
            <div v-if="selected.bullets?.length">
              ...
              <ul class="space-y-2">
                <li v-for="b in selected.bullets" :key="b" class="text-gray-300 text-sm font-exo flex gap-3">
                  <span class="text-cyan-400 flex-shrink-0 mt-0.5">▹</span>{{ b }}
                </li>
              </ul>
            </div>
            <div v-if="selected.tech_stack?.length">
              <p class="text-gray-500 text-xs font-exo uppercase tracking-wider mb-2">Technologies</p>
...
.modal-box { width: 100%; max-width: 600px; background: rgba(13,17,23,0.98); border: 1px solid rgba(34,211,238,0.2); }
