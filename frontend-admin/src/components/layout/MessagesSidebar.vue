          ...
          <aside v-if="modelValue" class="messages-drawer card-glass flex flex-col">
            ...
            <div class="flex items-center justify-between p-5 border-b border-cyan-400/20">
              <div class="flex items-center gap-3">
                ...
                <span v-if="store.unreadCount > 0" class="badge-unread">{{ store.unreadCount }} new</span>
              </div>
              <div class="flex items-center gap-2">
                <AppTooltip text="Mark all read">
                  <button @click="markAllRead"
                    class="text-gray-500 hover:text-cyan-400 transition-colors p-1.5 rounded text-xs">
                    ✓✓
                  </button>
                </AppTooltip>
            ...
            <div class="flex border-b border-cyan-400/10">
              <button v-for="tab in tabs" :key="tab.value"
                ...
                {{ tab.label }}
                <span v-if="tab.value === 'unread' && store.unreadCount > 0"
                  class="ml-1 text-cyan-400">({{ store.unreadCount }})</span>
              </button>
            </div>

            <!-- Messages list -->
            <div class="flex-1 overflow-y-auto">
              <div v-if="store.loading" class="flex flex-col items-center justify-center h-40 gap-3">
                <div class="loading-spinner" />
                <p class="text-gray-600 text-xs font-exo">Loading messages...</p>
              </div>

              <div v-else-if="filteredMessages.length === 0"
                class="flex flex-col items-center justify-center h-40 text-gray-600">
                <span class="text-3xl mb-2">✉</span>
                <p class="text-sm font-exo">
              ...
              <div v-else>
                <div
                  ...
                  :class="['msg-item px-5 py-4 border-b border-cyan-400/5 cursor-pointer transition-all group relative',
                    selectedId === msg.id ? 'bg-cyan-400/10' : 'hover:bg-white/5',
                    !msg.is_read ? 'border-l-2 border-l-cyan-400' : 'border-l-2 border-l-transparent']">
                  <div class="flex items-start justify-between gap-2">
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2">
                        <p :class="['text-sm font-exo truncate', !msg.is_read ? 'text-white font-semibold' : 'text-gray-300']">
                          {{ msg.sender_name }}
                        </p>
                        <span v-if="!msg.is_read" class="w-1.5 h-1.5 rounded-full bg-cyan-400 flex-shrink-0" />
                      </div>
                      <p class="text-gray-500 text-xs font-exo truncate">{{ msg.sender_email }}</p>
                      <p class="text-gray-400 text-xs font-exo mt-1 line-clamp-2">{{ msg.message }}</p>
                    </div>
                    <div class="flex flex-col items-end gap-2 flex-shrink-0">
                      <p class="text-gray-600 text-xs">{{ formatDate(msg.created_at) }}</p>
                      <button @click.stop="deleteMessage(msg.id)"
                        class="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-300 transition-all text-xs">
                        ✕
                      </button>
                    </div>
                  </div>
              ...
              <div v-if="selected" class="message-detail border-t border-cyan-400/20 p-5 bg-gray-900/50">
                <div class="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <a :href="`mailto:${selected.sender_email}`" class="text-cyan-400 text-xs hover:underline">
                      {{ selected.sender_email }}
                    </a>
                  </div>
                  <div class="flex items-center gap-2">
                    <p class="text-gray-600 text-xs font-exo">{{ formatDate(selected.created_at) }}</p>
                    <button @click="selected = null; selectedId = null"
                      class="text-gray-600 hover:text-gray-300 text-xs p-0.5">✕</button>
                  </div>
                </div>
                <p class="text-gray-300 text-sm font-exo leading-relaxed whitespace-pre-wrap">
                  {{ selected.message }}
                </p>
                <div class="flex gap-2 mt-4">
                  ...
                  <button @click="deleteMessage(selected.id)"
...
// Always re-fetch when the drawer opens so content is never stale.
// AdminHeader already calls fetch() before emitting open-messages,
// but this watch handles the case where the drawer is reopened
// without going through the bell button (e.g. keyboard shortcut).
watch(() => props.modelValue, async (opened) => {
  if (opened) {
    // Reset to "All" tab so user immediately sees all messages
    activeTab.value = 'all'
    selected.value = null
