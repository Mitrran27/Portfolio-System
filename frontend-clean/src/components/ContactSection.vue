    ...
    <div class="max-w-4xl mx-auto">
      ...
      <div class="text-center mb-12">
        <h2 class="font-orbitron font-bold text-gray-900" style="font-size:clamp(1.6rem,4vw,2.8rem)">
          <span class="text-blue-700 neon-text">Get in</span>
          <span class="text-gray-900"> Touch</span>
        </h2>
      ...
      <div :class="['grid grid-cols-1 sm:grid-cols-4 gap-4 mb-10 section-fade', visible ? 'visible' : '']">
        ...
        <a v-if="info?.email" :href="`mailto:${info.email}`"
          class="quick-card card-glass rounded-2xl p-5 text-center hover:border-blue-500/50 transition-all group">
          <div class="w-12 h-12 rounded-full bg-blue-50 border border-blue-500/30 flex items-center justify-center mx-auto mb-3 group-hover:bg-blue-500/10 transition-colors">
            <svg class="w-5 h-5 text-blue-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
            </svg>
          </div>
          <p class="text-gray-900 font-exo text-sm font-medium mb-1">Email Me</p>
          <p class="text-blue-700 text-xs font-exo truncate">{{ info.email }}</p>
        </a>

        <!-- Quick Chat card -->
        <div class="quick-card card-glass rounded-2xl p-5 text-center hover:border-red-500/50 transition-all group cursor-pointer"
          @click="scrollToForm">
          <div class="w-12 h-12 rounded-full bg-red-50 border border-red-500/30 flex items-center justify-center mx-auto mb-3 group-hover:bg-red-50 transition-colors">
            <svg class="w-5 h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
            </svg>
          </div>
          <p class="text-gray-900 font-exo text-sm font-medium mb-1">Quick Chat</p>
          <p class="text-red-600 text-xs font-exo">Send a message below</p>
        </div>

        <!-- Location  -->
        <div class="quick-card card-glass rounded-2xl p-5 text-center">
          <div class="w-12 h-12 rounded-full bg-emerald-50 border border-emerald-300 flex items-center justify-center mx-auto mb-3">
            <svg class="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
          </div>
          <p class="text-gray-900 font-exo text-sm font-medium mb-1">Based in</p>
          <p class="text-emerald-600 text-xs font-exo">{{ info?.location || 'Malaysia' }}</p>
          <span v-if="info?.available_for_work" class="inline-block mt-2 text-xs bg-emerald-50 border border-emerald-300 text-emerald-600 px-2 py-0.5 rounded-full font-exo">
            ● Open to work
          </span>
        </div>
        ...
        <!-- Availability card -->
        <div class="quick-card card-glass rounded-2xl p-5 text-center">
          <div class="w-12 h-12 rounded-full bg-amber-50 border border-amber-300 flex items-center justify-center mx-auto mb-3">
            <svg class="w-5 h-5 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
          </div>
          <p class="text-gray-900 font-exo text-sm font-medium mb-1">Availability</p>
          <span class="inline-block mt-2 text-xs bg-amber-50 border border-amber-300 text-amber-600 px-2 py-0.5 rounded-full font-exo">
            Consulting
          </span><br>
          <span class="inline-block mt-2 text-xs bg-amber-50 border border-amber-300 text-amber-600 px-2 py-0.5 rounded-full font-exo">
            Freelance
          </span><br>
          <span v-if="info?.available_for_work" class="inline-block mt-2 text-xs bg-amber-50 border border-amber-300 text-amber-600 px-2 py-0.5 rounded-full font-exo">
            ● Open to work
          </span>
        </div>
      </div>
      ...
      <div class="flex justify-center flex-wrap gap-4 mb-10">
        ...
        <a v-if="info?.email" :href="`mailto:${info.email}`"
          class="social-icon w-12 h-12 rounded-full card-glass flex items-center justify-center text-gray-400 hover:text-red-600 gradient-border">
      ...
      <div id="contact-form-wrapper" :class="['card-glass rounded-2xl p-6 md:p-8 gradient-border section-fade', visible ? 'visible' : '']"
        ...
        <h3 class="text-gray-900 font-orbitron text-sm mb-6">SEND A MESSAGE</h3>
        <form v-if="!submitted" @submit.prevent="sendMessage" class="space-y-5">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
              <label class="block text-blue-700 text-sm font-medium mb-2 font-exo">Your Name</label>
            ...
            <div>
              <label class="block text-blue-700 text-sm font-medium mb-2 font-exo">Your Email</label>
          ...
          <div>
            <label class="block text-blue-700 text-sm font-medium mb-2 font-exo">Message</label>
          ...
          <button type="submit" :disabled="sending"
            ...
            <span v-else class="flex items-center justify-center gap-2">
              <span class="inline-block w-4 h-4 border-2 border-blue-500/30 border-t-cyan-400 rounded-full animate-spin" />
              Sending...
            </span>
          </button>
        </form>
        ...
        <div v-else class="py-10 text-center space-y-4">
          <div class="w-16 h-16 rounded-full bg-blue-50 border border-blue-500/40 flex items-center justify-center mx-auto text-2xl">✨</div>
          <h3 class="text-gray-900 font-orbitron">Message Sent!</h3>
