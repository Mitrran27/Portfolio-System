<template>
  <section id="projects" class="py-20 px-4 bg-gray-50" ref="sectionRef">
    <div class="max-w-6xl mx-auto">
      <div class="text-center mb-12">
        <h2 class="font-orbitron font-bold text-gray-900" style="font-size:clamp(1.6rem,4vw,2.8rem)">
          <span class="text-blue-700 neon-text">Featured</span>
      ...
      <div v-if="!projects?.length" class="text-center text-gray-400 font-exo py-12">No projects added yet.</div>
        ...
        <div class="flex flex-wrap justify-center gap-2 mb-10">
          <button
            ...
            :class="activeCategory === cat.value
              ...
              : 'border-gray-200 bg-transparent text-gray-400 hover:border-gray-500 hover:text-gray-400'">
            {{ cat.emoji }} {{ cat.label }}
          ...
          <div :key="activeCategory" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
            <div v-for="(project, i) in filteredProjects" :key="project.id"
              ...
              <div class="project-img relative overflow-hidden"
                ...
                <div v-else class="w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-500/5 to-fuchsia-500/5">
                  <span class="text-4xl opacity-20">🚀</span>
                </div>

                <!-- Hover overlay -->
                <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                  <span class="text-blue-700 font-exo text-sm border border-blue-500/50 px-4 py-2 rounded-lg">View Details</span>
                </div>
                ...
                <div v-if="project.featured" class="absolute top-2 right-2 text-xs bg-blue-500/10 border border-blue-500/40 text-blue-700 px-2 py-0.5 rounded font-exo backdrop-blur-sm">
                  Featured
                </div>
              </div>
              ...
              <!-- Info -->
              <div class="p-4">
                <h3 class="text-gray-900 font-exo font-semibold text-sm mb-1">{{ project.title }}</h3>
    ...
    <Transition name="modal">
      <div v-if="activeProject" class="modal-backdrop" @click.self="closeModal">
        <div class="modal-box card-glass rounded-2xl overflow-hidden">
          <!-- Header -->
          <div class="flex items-center justify-between p-5 border-b border-blue-500/20">
            <div>
              <div class="flex items-center gap-2 flex-wrap">
                <h3 class="text-gray-900 font-orbitron font-bold text-base">{{ activeProject.title }}</h3>
                <!-- All type badges -->
              ...
              <div v-if="activeProject.featured" class="text-xs text-blue-700 font-exo mt-0.5">Featured Project</div>
            </div>
            <button @click="closeModal" class="text-gray-400 hover:text-gray-900 transition-colors p-1 rounded ml-4">
          ...
          <div v-if="allImages.length" class="screenshot-area relative bg-black/50 overflow-hidden">
            ...
            <button v-if="allImages.length > 1" @click="prevSlide"
              class="absolute left-2 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-black/80 text-gray-900 w-8 h-8 rounded-full flex items-center justify-center transition-all">‹</button>
            <button v-if="allImages.length > 1" @click="nextSlide"
              class="absolute right-2 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-black/80 text-gray-900 w-8 h-8 rounded-full flex items-center justify-center transition-all">›</button>
            ...
            <div v-if="allImages.length > 1" class="absolute top-2 right-2 bg-black/60 text-gray-900 text-xs font-exo px-2 py-0.5 rounded">
          ...
          <div v-else class="screenshot-area flex items-center justify-center bg-gradient-to-br from-blue-500/5 to-fuchsia-500/5">
            <span class="text-6xl opacity-20">🚀</span>
          </div>

          <!-- Thumbnail strip -->
          <div v-if="allImages.length > 1" class="flex gap-2 px-4 py-2 overflow-x-auto bg-gray-50/80 border-b border-blue-500/10">
            <button v-for="(img, idx) in allImages" :key="idx" @click="activeSlide = idx"
              :class="['flex-shrink-0 w-14 h-10 rounded overflow-hidden border-2 transition-all',
                idx === activeSlide ? 'border-blue-500' : 'border-transparent opacity-50 hover:opacity-80']">
          ...
          <div class="p-5 space-y-4">
            <p class="text-gray-400 font-exo text-sm leading-relaxed">{{ activeProject.description }}</p>
            <div v-if="activeProject.tech_stack?.length">
              <p class="text-gray-400 text-xs font-exo uppercase tracking-wider mb-2">Tech Stack</p>
            ...
            <div class="flex gap-3 pt-2">
              ...
              <a v-if="activeProject.github_url" :href="activeProject.github_url" target="_blank"
                class="flex-1 py-2.5 rounded-xl text-xs font-orbitron text-center border border-red-500/40 text-red-600 hover:bg-red-50 transition-all"
      ...
      <div v-if="lightboxSrc" class="lightbox-backdrop" @click="closeLightbox">
        <div class="lightbox-inner" @click.stop>
          ...
          <button @click="closeLightbox"
            class="absolute top-4 right-4 w-10 h-10 bg-black/60 hover:bg-black/90 text-gray-900 rounded-full flex items-center justify-center text-xl transition-all">
            ✕
          </button>
...
const typeIconBg = (type) => {
  const map = { 'web-app': 'bg-blue-500/15 text-blue-700', 'mobile-app': 'bg-red-500/15 text-red-600', 'website': 'bg-amber-100 text-amber-700', 'ui-ux': 'bg-pink-100 text-pink-700', 'ai': 'bg-violet-100 text-violet-700' }
...
const typeStyle = (type) => {
  const map = { 'web-app': 'bg-blue-500/10 text-blue-700 border border-blue-500/30', 'mobile-app': 'bg-red-50 text-red-600 border border-red-500/30', 'website': 'bg-amber-50 text-amber-700 border border-amber-200', 'ui-ux': 'bg-pink-50 text-pink-700 border border-pink-200', 'ai': 'bg-violet-50 text-violet-700 border border-violet-200' }
...
<style scoped>
.project-img { height: 180px; background: #f1f5f9; }
